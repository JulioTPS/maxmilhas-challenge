...
public class CpfService {
    ...
    public Cpf findByCpfNumber(String cpfNumber) {
        if(!isCPF(cpfNumber)){
            throw new IllegalArgumentException("InvalidCpfException");
        }
        List<Cpf> cpfs = cpfRepository.findByCpf(cpfNumber);
        if(cpfs.isEmpty()){
            throw new IllegalStateException("NotFoundCpfException");
        }
        return cpfs.get(0);
    };
    ...
    public Cpf save(Cpf cpf) {
        ...
        List<Cpf> cpfs = cpfRepository.findByCpf(cpf.getCpf());
        if(!cpfs.isEmpty()){
            throw new IllegalStateException("ExistsCpfException");
        }
